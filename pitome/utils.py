...
def parse_keep_ratio(num_layers: int, r: Union[List[float], Tuple[float, float], float]) -> List[float]:
    """Return a per‑layer list of *keep‑ratios* (floats 0<r≤1).

    * **float** → constant ratio.
    * **tuple(float, float)** → (ratio, inflect) ; inflect∈[‑1,+1] describes linear trend.
    * **list[float]** → literal schedule, padded with 1’s if shorter.
    """
    if isinstance(r, list):
        if len(r) < num_layers:
            r = r + [1.0] * (num_layers - len(r))
        return [float(max(1e-2, min(0.95, v))) for v in r]
    ...
    base = float(max(1e-2, min(0.95, base)))
    inflect = max(-1.0, min(1.0, float(inflect)))

    start = base * (1.0 - inflect)
    end   = base * (1.0 + inflect)
    step  = (end - start) / max(1, num_layers - 1)
    return [float(max(1e-2, min(0.95, start + step * i))) for i in range(num_layers)]